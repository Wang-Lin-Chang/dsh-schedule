...
export class ScheduleRegistry {
    ...
    create(input) {
        if (input.prompt.trim().length === 0)
            throw new Error('invalid_prompt: expected a non-empty prompt');
        ...
        if (!Number.isSafeInteger(scheduledAt) || scheduledAt <= this.nowFn())
            throw new Error('not_future: scheduledAt must be a future instant');
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?)`)
            .run(id, input.action, input.rule, input.prompt, input.jobSpec ? JSON.stringify(input.jobSpec) : null, scheduledAt, input.everySeconds ?? null, 'scheduled', input.createdBy ?? null, this.nowFn());
    ...
    delete(id) {
        ...
        const r = this.get(id);
        if (r === undefined || r.status === 'dispatched' || r.status === 'cancelled')
    ...
    nextId() {
        const row = this.db.prepare(`INSERT INTO schedule_counters (kind,count) VALUES ('sch',1) ON CONFLICT(kind) DO UPDATE SET count=count+1 RETURNING count`).get();
        return row.count;
    }
    row(r) {
        return {
    ...
    claim(id, now, who = this.who, leaseMs = this.leaseMs) {
    ...
    executeAction(rec) {
        ...
        try {
            if (rec.action === 'job')
                return this.executor.executeJob(rec);
    ...
    startDriver() {
        ...
        this.pollTimer = setInterval(() => { try {
            this.sweep();
        }
        catch { } }, this.fallbackPollMs);
    ...
    scheduleNextTimer() {
        ...
        const row = this.db.prepare("SELECT scheduled_at FROM schedules WHERE status='scheduled' ORDER BY scheduled_at LIMIT 1").get();
        if (row === undefined)
            return;
