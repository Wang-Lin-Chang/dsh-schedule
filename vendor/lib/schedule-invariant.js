 ...
 */
export function validateSchedule(rec, fail) {
    const id = String(rec.id);
    if (!SCHEDULE_ID.test(id))
        fail(`schedule id ${JSON.stringify(id)} must be sch-<positive ordinal>`);
    if (rec.prompt.trim().length === 0)
        fail(`schedule ${JSON.stringify(id)} prompt must be non-empty`);
...
/** 批量校验：对列表逐条 fail 收集 */
export function validateSchedules(recs, fail) {
    for (const rec of recs)
        validateSchedule(rec, fail);
}
