...
export function parseAbsolute(at, timeZone) {
    if (RFC3339.test(at)) {
    ...
    if (LOCAL.test(at)) {
        if (typeof timeZone !== 'string' || timeZone.length === 0)
            throw new Error('invalid_time_zone: local at requires time_zone');
...
export function zonedToEpoch(local, tz) {
    ...
    if (!m)
        throw new Error('invalid_selector: local form requires YYYY-MM-DDTHH:mm:ss');
    ...
    if (backStr !== local)
        throw new Error('invalid_selector: local time falls in a DST gap or is ambiguous');
    return utcMs;
}
